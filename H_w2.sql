...
SELECT mark, color, COUNT(color) AS 'Количество авто' FROM auto
WHERE mark = 'LADA'
GROUP BY color, mark
UNION SELECT mark, color, COUNT(color) AS 'Количество авто' FROM auto
WHERE mark = 'BMW'
GROUP BY color, mark;

-- Вывести на экран марку авто и количество AUTO не этой марки
SELECT DISTINCT mark, (SELECT count(1) FROM auto a1 WHERE a1.mark != a.mark) FROM auto a;
